...
const Dashboard: FC = () => {
  const cardsMap = cards.map((card) => {
    return (
      <div key={card.route} className="flex flex-col p-4 text-center border rounded-md">
        ...
        <Link to={card.route}>
          <button className="mt-2 button-bg">Check out</button>
        </Link>
        <p className="mt-8 text-sm font-light">{card.description}</p>
  ...
  return (
    <main className="flex justify-center mt-24 xl:mt-48">
      <section className="flex flex-col w-11/12 gap-10 p-2 xl:flex-row">{cardsMap}</section>
    </main>
  );
};
