    ...
    <div className={`border rounded flex flex-col p-3`}>
      <div className=''>
        <div className='flex flex-row justify-end gap-4'>
          {text.context ? 
            <button onClick={toggleContext} className='text-gray-300 underline duration-100 hover:text-gray-600'>
              View context
            </button>
          : null}
      ...
      <div className={`${isContextHidden ? 'hidden' : ''} ${text.isClosed && 'line-through'}`}>{text.context}</div>
    </div>
  );
};
